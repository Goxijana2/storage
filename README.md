This repository hosts the [Storage Standard](https://storage.spec.whatwg.org/).
...
Issues labeled ["good first issue"](https://github.com/whatwg/storage/labels/good%20first%20issue) are a good place to get a taste for editing the Storage Standard. Note that we don't assign issues and there's no reason to ask for availability either, just provide a pull request.
...
In short, change `storage.bs` and submit your patch, with a [good commit message](https://github.com/whatwg/meta/blob/main/COMMITTING.md).
...
To ensure your patch meets all the necessary requirements, please also see the [Contributor Guidelines](https://github.com/whatwg/meta/blob/main/CONTRIBUTING.md). Editors of the Storage Standard are expected to follow the [Maintainer Guidelines](https://github.com/whatwg/meta/blob/main/MAINTAINERS.md).

## Tests

Tests are an essential part of the standardization process and will need to be created or adjusted as changes to the standard are made. Tests for the Storage Standard can be found in the `storage/` directory of [`web-platform-tests/wpt`](https://github.com/web-platform-tests/wpt).

A dashboard showing the tests running against browser engines can be seen at [wpt.fyi/results/storage](https://wpt.fyi/results/storage).
